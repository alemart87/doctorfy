...
def analyze_food():
    ...
    try:
        ...
        # Verificar si tiene suficientes créditos (1 crédito por análisis)
        if user.credits < 1:
            return jsonify({
                'error': 'Sin créditos suficientes',
                'message': 'No tienes créditos suficientes para realizar el análisis',
                'redirect_to': '/credits-info'
            }), 403
