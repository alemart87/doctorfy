...
const Nutrition = () => {
  ...
  const handleAnalyzeFood = async () => {
    ...
    try {
      ...
      if (err.response?.status === 403 && err.response?.data?.redirect_to === '/credits-info') {
        setError(
          <Box sx={{ textAlign: 'center' }}>
            ...
            <Typography variant="body1" gutterBottom>
              Necesitas 1 crédito para realizar un análisis nutricional
            </Typography>
            <Button 
              variant="contained" 
              color="primary"
      ...
      setAnalysisModal({ open: false, status: 'idle' });
    }
  };
